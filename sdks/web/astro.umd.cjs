...
`;class f{constructor(e){this.overlay=null,this.styleEl=null,this.step="alias",this.aliasValue="",this.errorMsg="",this.sessionData=null,this.options=e}mount(){this.injectStyles(),this.render()}destroy(){var e,t;(e=this.overlay)==null||e.remove(),(t=this.styleEl)==null||t.remove(),this.overlay=null}injectStyles(){document.getElementById("ow-styles")||(this.styleEl=document.createElement("style"),this.styleEl.id="ow-styles",this.styleEl.textContent=S,document.head.appendChild(this.styleEl))}render(){var i,n,a;(i=this.overlay)==null||i.remove();const e=this.resolveTheme(),t=document.createElement("div");t.className="ow-overlay",t.addEventListener("click",l=>{l.target===t&&this.handleDismiss()});const s=document.createElement("div");s.className="ow-modal",s.setAttribute("data-theme",e),s.style.setProperty("--ow-accent",this.safeColor((n=this.options.branding)==null?void 0:n.accentColor)??"#07315F"),s.innerHTML=this.renderHTML(),t.appendChild(s),document.body.appendChild(t),this.overlay=t,this.bindEvents(s),(a=s.querySelector(".ow-alias-input"))==null||a.focus()}renderHTML(){var y,v;const e=this.options.branding??{},t=this.escapeHtml(e.gatewayName??"OpenWave"),s=this.escapeHtml(e.merchantName??"Merchant checkout"),i=this.escapeHtml(e.acquirerName??"Bank-authenticated payment"),n=this.safeUrl(e.gatewayLogoUrl)??x,a=this.safeUrl(e.openWaveLogoUrl)??g,l=this.safeUrl(e.neptuneLogoUrl)??k,d=this.safeUrl(e.merchantLogoUrl),r=this.safeUrl(e.acquirerLogoUrl),c=d??n,o=(y=this.sessionData)==null?void 0:y.amount,u=((v=this.sessionData)==null?void 0:v.currency)??"LYD",M=u==="LYD"?1e3:100,T=u==="LYD"?3:2,b=o!=null?`<div class="ow-amount"><span class="ow-amount-value">${(o/M).toFixed(T)}</span><span class="ow-amount-currency">${u}</span></div>`:"",$=`
      <div class="ow-header">
        <div class="ow-brand">
    ...
    `,U=`<div class="ow-footer"><div class="ow-footer-logos"><img src="${this.escapeHtml(l)}" alt="Neptune" /><img src="${this.escapeHtml(a)}" alt="OpenWave" /></div>Secured by <strong>${t}</strong>. The merchant never sees bank credentials.</div>`;let m="";return this.step==="alias"?m=`
        <div class="ow-body">
          ${b}
      ...
      `:this.step==="processing"?m='<div class="ow-body"><div class="ow-state"><span class="ow-spinner"></span><div class="ow-state-title">Processing payment</div><div class="ow-state-subtitle">Your bank is completing the authorization.</div></div></div>':this.step==="success"?m='<div class="ow-body"><div class="ow-state"><div class="ow-state-mark">OK</div><div class="ow-state-title">Payment successful</div><div class="ow-state-subtitle">The merchant has received the confirmed payment status.</div></div><button class="ow-btn ow-done-btn">Done</button></div>':this.step==="failed"&&(m=`<div class="ow-body"><div class="ow-state"><div class="ow-state-mark fail">!</div><div class="ow-state-title">Payment failed</div><div class="ow-state-subtitle">${this.escapeHtml(this.errorMsg||"The bank could not complete this payment.")}</div></div><button class="ow-btn ow-retry-btn">Try again</button></div>`),$+C+m+U}bindEvents(e){var t,s,i,n,a,l,d,r,c;(t=e.querySelector(".ow-close"))==null||t.addEventListener("click",()=>this.handleDismiss()),(s=e.querySelector(".ow-cancel-btn"))==null||s.addEventListener("click",()=>this.handleDismiss()),(i=e.querySelector(".ow-pay-btn"))==null||i.addEventListener("click",()=>this.handleAliasContinue(e)),(n=e.querySelector(".ow-alias-input"))==null||n.addEventListener("keydown",o=>{o.key==="Enter"&&this.handleAliasContinue(e)}),(a=e.querySelector(".ow-confirm-btn"))==null||a.addEventListener("click",()=>this.handleOtpConfirm(e)),(l=e.querySelector(".ow-otp-input"))==null||l.addEventListener("keydown",o=>{o.key==="Enter"&&this.handleOtpConfirm(e)}),(d=e.querySelector(".ow-back-btn"))==null||d.addEventListener("click",()=>{this.step="alias",this.errorMsg="",this.render()}),(r=e.querySelector(".ow-done-btn"))==null||r.addEventListener("click",()=>this.destroy()),(c=e.querySelector(".ow-retry-btn"))==null||c.addEventListener("click",()=>{this.step="alias",this.errorMsg="",this.render()})}async handleAliasContinue(e){var i;const t=e.querySelector(".ow-alias-input");if(this.aliasValue=(t==null?void 0:t.value.trim())??"",!this.aliasValue){this.errorMsg="Please enter an alias or IBAN",this.render();return}const s=e.querySelector(".ow-pay-btn");s.disabled=!0,s.innerHTML='<span class="ow-spinner"></span>Verifying',this.errorMsg="";try{const n=this.options.gatewayUrl??"",l=this.aliasValue.toUpperCase().startsWith("LY")?{payer_iban:this.aliasValue}:{payer_alias:this.aliasValue},d=await fetch(`${n}/payments/sessions/${this.options.sessionId}/resolve-payer`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(l)}),r=await d.json();if(!d.ok){this.errorMsg=(r==null?void 0:r.message)??((i=r==null?void 0:r.error)==null?void 0:i.message)??"Verification failed",this.render();return}const c=await fetch(`${n}/payments/sessions/${this.options.sessionId}/select-auth`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({auth_mode:"OTP"})}),o=await c.json().catch(()=>({}));if(!c.ok){this.errorMsg=(o==null?void 0:o.message)??(o==null?void 0:o.error)??"Could not send OTP",this.render();return}this.sessionData={...this.sessionData,...r},this.step="otp",this.render()}catch{this.errorMsg="Network error. Please try again.",this.render()}}async handleOtpConfirm(e){var s,i,n,a,l,d;const t=((s=e.querySelector(".ow-otp-input"))==null?void 0:s.value.trim())??"";if(t.length<4){this.errorMsg="Please enter a valid OTP",this.render();return}this.step="processing",this.render();try{const r=this.options.gatewayUrl??"",c=await fetch(`${r}/payments/sessions/${this.options.sessionId}/confirm`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({otp_code:t})}),o=await c.json();if(!c.ok){this.step="failed",this.errorMsg=(o==null?void 0:o.message)??((i=o==null?void 0:o.error)==null?void 0:i.message)??"Payment failed",this.render(),(a=(n=this.options).onFailed)==null||a.call(n,{session_id:this.options.sessionId,status:"FAILED",message:this.errorMsg});return}this.step="success",this.render(),(d=(l=this.options).onSuccess)==null||d.call(l,{session_id:this.options.sessionId,status:"COMPLETED",reference:o.transfer_reference??o.reference,receipt_url:o.receipt_url})}catch{this.step="failed",this.errorMsg="Network error. Please try again.",this.render()}}handleDismiss(){var e,t;this.destroy(),(t=(e=this.options).onDismiss)==null||t.call(e)}resolveTheme(){var t;const e=this.options.theme??"auto";return e==="light"?"light":e==="dark"||(t=window.matchMedia)!=null&&t.call(window,"(prefers-color-scheme: dark)").matches?"dark":"light"}escapeHtml(e){return e.replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]??t)}safeUrl(e){if(!e)return null;try{const t=new URL(e);return["https:","http:"].includes(t.protocol)?t.toString():null}catch{return null}}safeColor(e){return e&&/^#[0-9a-fA-F]{6}$/.test(e)?e:null}}let p=null;function E(w){p&&p.destroy(),p=new f(w),p.mount()}function L(){p==null||p.destroy(),p=null}h.AstroCheckout=f,h.checkout=E,h.dismiss=L,Object.defineProperty(h,Symbol.toStringTag,{value:"Module"})});
